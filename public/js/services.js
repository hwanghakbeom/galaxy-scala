...
angular.module('myApp.services', []).
  ...
  service('API', ['$resource', function($resource) {
    ...
    this.getUserReports = function(option,selected, date, date2, callback){

      var api = $resource('project_report/:projectid/:year/:month', {
      ...
      if(option == "CurrentMonth"){
        api = $resource('project_report/:projectid/currentMonth', {
        projectid: selected.id
        },{
          query: {method: 'GET', isArray: true}
        });
      ...
      if(option == "LastMonth"){
        api = $resource('project_report/:projectid/lastMonth', {
        projectid: selected.id
        },{
          query: {method: 'GET', isArray: true}
        });   
      ...
      if(option == "LifeCyle"){
        api = $resource('project_report/:projectid/lifeCycle', {
        projectid: selected.id
        },{
          query: {method: 'GET', isArray: true}
        });  
      ...
      if(option == "CustomDate"){
        api = $resource('project_report/:projectid/CustomDate/:startDate/:endDate', {
        projectid: selected.id,
        startDate:date.getFullYear().toString() + (date.getMonth() + 1 ).toString() + date.getDate().toString(),
        endDate:date2.getFullYear().toString() + (date2.getMonth() + 1).toString() + date2.getDate().toString()
        },{
          query: {method: 'GET', isArray: true}
        });
      }
